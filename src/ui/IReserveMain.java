...
public class IReserveMain {
	...
	CustomerCatalog bookings = new CustomerCatalog();
	AirplaneInterface plane = new Airplane(747, "Boeing", 10);
	...
	while (whatToDo != 8) {
		...
		switch (whatToDo) {
			...
			if (classChoice.equals("F")) {
			    Ticket ticket = new Ticket(TravelClass.FIRST, 20000);
			    System.out.println("Ticket price is: " + numberFormatter.format(ticket.getTicketPrice()));
			    System.out.println("Checking for available seats in " + ticket.getTicketClass() + " CLASS.");
			} else if (classChoice.equals("E")) {
			    Ticket ticket = new Ticket(TravelClass.ECONOMY, 5000);
			    System.out.println("Ticket price is: " + numberFormatter.format(ticket.getTicketPrice()));
			    System.out.println("Checking for available seats in " + ticket.getTicketClass() + " CLASS.");
			}
//					plane.;
